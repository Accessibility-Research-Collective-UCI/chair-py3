...
f_scores = [
    SCORES_PATH + "hallucinated_words_baseline-gan_beam5_test.json",
...
corr_s_cs_s = [0] * len(
    SCORES
)  # correlation between sentence scores+(1-CHs) and human scores
corr_s_ci_s = [0] * len(
    SCORES
)  # correlation between sentence scores+(1-CHi) and human scores
...
for im in range(NIMAGES):
    ...
    for metric in range(len(SCORES)):
        corr = np.corrcoef(
            correctForNan([x[metric] for x in s_m]),
            correctForNan([x[0] for x in h_s_m]),
        )[0][1]
    ...
    for metric in range(len(SCORES)):
        # ch_s
        corr = np.corrcoef(
            correctForNan([x[metric] + c_m[i][0] for i, x in enumerate(s_m)]),
            correctForNan([x[0] for x in h_s_m]),
        )[0][1]
        corr_s_cs_s[metric] += corr
        # ch_i
        corr = np.corrcoef(
            correctForNan([x[metric] + c_m[i][1] for i, x in enumerate(s_m)]),
            correctForNan([x[0] for x in h_s_m]),
        )[0][1]
...
for metric in [5, 6, 7]:  # focus on 'M', 'C', 'S'
    print(f"{SCORES[metric]}\t{corr_s_s[metric] / float(NIMAGES):0.04f}")

for metric in [5, 6, 7]:  # focus on 'M', 'C', 'S'
    print(
        f"{SCORES[metric] + '+' + CHAIR[0]}\t{corr_s_cs_s[metric] / float(NIMAGES):0.04f}"
    )

for metric in [5, 6, 7]:  # focus on 'M', 'C', 'S'
    print(
        f"{SCORES[metric] + '+' + CHAIR[1]}\t{corr_s_ci_s[metric] / float(NIMAGES):0.04f}"
    )
