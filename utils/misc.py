...
def get_consistency(tag, annotation_path, robust=False):
    ...
    hallucinated_json = f"./output/hallucination/hallucinated_words_{tag}.json"

    if not os.path.exists(hallucinated_json):
        print(f"Computing hallucination file for tag {tag}")
        sentence_template = "generated_sentences/{}.json"
        _, imids, _ = load_generated_captions(sentence_template.format(tag))
        evaluator = CHAIR(imids, annotation_path)
        evaluator.get_annotations()
        cap_dict = evaluator.compute_chair(sentence_template.format(tag))
        save_hallucinated_words(sentence_template.format(tag), cap_dict)
