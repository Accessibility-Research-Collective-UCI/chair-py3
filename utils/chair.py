...
class CHAIR(object):
    def __init__(self, imids, coco_path):
        ...
        """
        synonyms = [
            s.strip().split(", ") for s in synonyms.split("\n") if s.strip() != ""
        ]
...
def load_generated_captions(cap_file):
    ...
    try:
        ...
        raise Exception(
            f"Expect caption file to consist of a dectionary with sentences correspdonding to the key 'imgToEval': {e}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_file", type=str, default="output/hallucinated_words.json"
    )
